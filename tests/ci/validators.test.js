...
function writeCatalogFixture(testDir, options = {}) {
  ...
  return { agentsPath, pluginJsonPath, marketplaceJsonPath };
}
...
function runTests() {
  ...
  if (test('fails when AGENTS catalog counts drift', () => {
    const testDir = createTestDir();
    const { agentsPath, pluginJsonPath, marketplaceJsonPath } = writeCatalogFixture(testDir, {
  ...
  if (test('syncs tracked catalog docs in write mode', () => {
    const testDir = createTestDir();
    const { agentsPath, pluginJsonPath, marketplaceJsonPath } = writeCatalogFixture(testDir, {
